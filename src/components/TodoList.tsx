    ...
    <div className="w-full flex flex-col">
      <div>
        <div className="flex justify-between">
          ...
          <button className="bg-blue h-[58px] px-7 rounded-xl addButton text-white" onClick={() => props.onAddNewTaskRequest()}>+ Add new task</button>
        </div>

        <ul>
          {todosToShow.map((todo) => (<TodoItem key={todo.id} todo={todo}></TodoItem>))}
        </ul>
      </div>

      {showEmptyState && <div className="flex flex-col justify-center items-center">
        <EmptyIcon />
